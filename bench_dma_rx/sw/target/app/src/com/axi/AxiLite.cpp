 ...
 * @return None
****************************************************************************/
AxiLite::AxiLite(uint64_t offset, uint32_t range, uint16_t nb_regs_write, uint16_t nb_regs_read){
    int r;
    int fd;
    ...
    if(fd != -1){
        ...
        uint32_t* axi_regs_u32_vptr = (uint32_t *)mmap(NULL, range, PROT_READ|PROT_WRITE, MAP_SHARED, fd, offset);
        r = (axi_regs_u32_vptr == NULL) ? EXIT_FAILURE : EXIT_SUCCESS;
        statusPrint(r, "Map pointer to virtual address of axi lite");
    
        _axi_regs_base = reinterpret_cast<std::atomic<uint32_t>*>(axi_regs_u32_vptr);
...
int AxiLite::write(uint32_t wdata, uint16_t regw){
    _axi_regs_base[regw].store(wdata, std::memory_order_relaxed);
    return EXIT_SUCCESS;
}
...
int AxiLite::write(float wdata, uint16_t regw, uint8_t fp_int, uint8_t fp_dec){
    return write(float2sfi(wdata, fp_int, fp_dec), regw);;
}
...
uint32_t AxiLite::read(uint16_t regr){
    return _axi_regs_base[regr].load(std::memory_order_relaxed);
}
...
float AxiLite::read(uint16_t regr, uint8_t fp_int, uint8_t fp_dec){
    return sfi2float(read(regr), fp_int, fp_dec);
}
