...
source "$BENCHDMARX_PATH/firmware/deactivate.sh"
source "$BENCHDMARX_PATH/firmware/activate.sh"

# Debug mode
target_exec=""
case $arg_debug_mode in
    true)
        fpath_exec="$BENCHDMARX_PATH/app/build/debug/bench_dma_rx.out"
        if [ ! -f $fpath_exec ]; then
            echo "Debug target executable not found, please run ./build.sh debug"
        fi
        ;;
    false)
        fpath_exec="$BENCHDMARX_PATH/app/build/release/bench_dma_rx.out"
        if [ ! -f $fpath_exec ]; then
