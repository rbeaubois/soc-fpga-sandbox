...
TAG_COLOR="\e[95m"
END_COLOR="\e[0m"
TAG_NAME="Bench DMA RX build"
TAG="[$TAG_COLOR$TAG_NAME$END_COLOR]"

# Define environment variable for BENCHDMARX_PATH if not already set
if [ -z "$BENCHDMARX_PATH" ]; then
    echo -e "$TAG Error: BENCHDMARX_PATH is not set"
    exit 1
fi
...
# Define directories
DRIVERS_DIR="$BENCHDMARX_PATH/drivers"
FIRMWARE_DIR="$BENCHDMARX_PATH/firmware"
APP_DIR="$BENCHDMARX_PATH/app"
...
clean() {
    ...
    sudo rm -r "/lib/firmware/xilinx/$BENCHDMARX_TARGET-bench_dma_rx"

    echo -e "$TAG Clean environment variables"
    unset "$BENCHDMARX_PATH"
